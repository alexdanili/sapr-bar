...
void PostProcessor::setProcessor(Processor* _processor)
{
    ...
    for(int i = 0; i < percentPoint(1,1).x()/50; i++)
    {
        uxscene->addLine(i*60,percentPoint(0,0.5).y(),50 + i*60,percentPoint(0,0.5).y(), QPen(Qt::red));
    }

    sxscene->clear();
    for(int i = 0; i < percentPoint(1,1).x()/50; i++)
    {
        sxscene->addLine(i*60,percentPoint(0,0.5).y(),50 + i*60,percentPoint(0,0.5).y(), QPen(Qt::red));
    }
